...
`omp-context` 管理 agent 对 OMP 会话历史的注意力分配。本文件只定义领域语言；`docs/acm-judgment-contract.md` 是判断语义的 canonical source，`skills/context-management/CORE.md` 是其面向模型的投影，`TOOL-CONTRACTS.md` 只拥有工具 mechanics 与调用时 guidance。

...
**状态**: Archival host framing confirmed; matching-`tool_execution_end` direct replacement rejected; OMP idle `session_stop` boundary adopted

OMP 的 post-travel Context Packet 使用受 provenance 约束的 continuation framing，transient travel result 不属于 rebuilt branch。若 matching `tool_execution_end` 立刻将 native AgentSession 换成 traveled branch 的 rebuilt packet，会在当前 tool/result 连续性结束前切断 live chronology。
...
**底层机制**:
OMP 的 SessionManager 会把 branch summary 投影到 session context；ACM 再把可信的 canonical summary 投影为 hidden `acm:continuation`。matching `tool_execution_end` direct replacement 会切断 originating assistant run 及 automatic retry/tool loop，因此 native replacement 必须等到宿主 settle 边界。
...
- 对照 matching-`tool_execution_end` direct replacement 与 OMP idle `session_stop` replacement；
...
**当前解决方式**:
Runtime 在 canonical handoff 中写入 versioned marker，并在 persisted summary details 中记录 `kind: acm_travel` 与 `handoffVersion`。所有 LLM-bound rebuild/normalization 路径只有在 marker、summary source、timestamp 与 persisted ACM travel provenance 同时匹配时，才把 summary 投影为 hidden `acm:continuation` custom message。`tool_result` interception 可被后置 extension 改写，因此不授权 provider cutover；下一次 `context` 只认 finalized matching non-error applied `toolResult`，随后直接以 Context Packet 作为唯一 NEXT authority。明确成功时，live adapter 为同一 SessionManager 记录 ticket；matching `tool_execution_end` 只保留 ticket。只有 OMP `session_stop` 且 `ctx.isIdle()` 为真、`ctx.hasPendingMessages()` 为假时，才从最新 verified active leaf rebuild 并替换 native AgentSession；notification-only `agent_end` 不得提前 release。`indeterminate` mutation 只 schedule persistent observation rebuild；明确失败或 `not_applied` 不 schedule。adapter unavailable/failed 不回滚已验证 travel。

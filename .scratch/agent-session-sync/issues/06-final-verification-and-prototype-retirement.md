...
Resolved with the pinned OMP 16.4.5 acceptance matrix in `test/host-fixture/travel-live-sync.test.ts`, `agent-session-adapter.test.ts`, and `compaction-lifecycle.test.ts`. The primary scenario now proves that the pre-travel stored message estimate crosses the native compaction threshold, the synchronized traveled branch falls below it, no synthetic `pre-compact-*` checkpoint is created by travel, the in-flight tool pair remains valid, and provider context/tree recovery remain truthful. Repeated travel, off-path restore, resume, failure, unsupported-host, lifecycle, and two-session parent/subagent isolation are covered by the same pinned host fixture suite.

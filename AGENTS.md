...
5. `runtime-lifecycle.ts` 在每次 LLM 调用前通过公开的 compaction-aware `buildSessionContext()` 重建 messages；失败最多重试 3 次，HUD 显示原因与进度。`session_start` / `session_switch` / `session_branch` / `session_tree` 重建当前 active branch 的 reminder state；`session_shutdown` 清理易失状态；`session_compact` 额外开启 baseline-only reminder cycle。
...
成功 `acm_travel` 与 `session_compact` 都把 reminder cycle 重置为 `baselinePending`。第一条真实 post-transition `turn_end` 用 assistant prompt usage 建立基线，并通过 `pi.appendEntry("acm:context-usage-state", ...)` 持久化但不进入 LLM context；后续隐藏 `acm:context-usage-reminder` custom message 记录已交付档位。`session_start`、`session_switch`、`session_branch`、`session_tree` 扫描 active branch 上最近 ACM travel/native compaction 之后的两类 entry，恢复 baseline 与最高已交付档位。

### 没有 /acm command

当前代码没有 `/acm` command，也不调用 command-only 的 `navigateTree()`。`session_stop` 不是 ACM continuation hook；它只为无工具的 reminder fallback 标记一次 terminal eligibility。
...
运行时 schema 仍来自 `pi.zod`，不要改成独立导入的 zod 实例。`acm_timeline` 为兼容 OMP 16.4.8 的 root-object registration 保持单对象 schema，并用 `superRefine` 恢复 view-specific required/forbidden field contract；不要退回 root discriminated union，也不要只依赖 execute-time 检查。

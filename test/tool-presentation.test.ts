    ...
    registerTool(tool: CapturedTool) {
      captured = {
        ...
        renderCall: tool.renderCall
          ? (args: unknown, theme: Theme, context: unknown) => tool.renderCall!(args, context, theme)
          : undefined,
      };
    },
...
describe("ACM tool OMP metadata", () => {
  test.each([checkpoint, timeline, travel])("$name uses the OMP tool surface without Pi-only prompt metadata", (tool: CapturedTool) => {

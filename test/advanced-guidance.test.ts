...
describe("advanced guidance availability", () => {
  test("uses OMP command discovery as the only availability source", () => {
  ...
  test("falls back to the static cue when the router path is missing, ambiguous, or unreadable", () => {
    ...
    expect(getAvailableAdvancedGuidance({
      getCommands: () => [
        { name: "skill:context-management", path: "/one/SKILL.md" },
        { name: "skill:context-management", path: "/two/SKILL.md" },
      ] as never,
    }, staticCue)).toBe(staticCue);
    expect(getAvailableAdvancedGuidance({
      getCommands: () => [{
        name: "skill:context-management",
        get path() { throw new Error("path unavailable"); },
      }] as never,
    }, staticCue)).toBe(staticCue);
  });

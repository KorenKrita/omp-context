...
describe("canonical handoff", () => {
  test("keeps the four supporting fields optional on the wire", () => {
    ...
    // The travel tool builds its zod schema from these shared descriptions,
    // so the optional fields must announce themselves as optional.
    const optional = ["evidence", "external", "exclusions", "recover"] as const;
    for (const field of optional) expect(HANDOFF_FIELD_DESCRIPTIONS[field]).toMatch(/^Optional:/);
    for (const field of ["goal", "state", "next"] as const) {
      expect(HANDOFF_FIELD_DESCRIPTIONS[field]).not.toMatch(/^Optional:/);
    }
  });

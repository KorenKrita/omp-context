...
import type { AgentMessage, ToolResultMessage } from "@oh-my-pi/pi-agent-core";
import {
  AgentSession,
  SessionManager,
  type ExtensionAPI,
...
function createFixture(
  ...
  // Pi is dispatching session_stop.
...
describe("deferred post-travel delivery on exact Pi host", () => {
  ...
  test("preserves same-run messages and tool pair through retry; session_stop replaces native context and later provider contexts stay current", async () => {
    AgentSession.prototype.getContextUsage = function () {
      return { tokens: 1_000, contextWindow: 100_000, percent: 1 };
    };
  ...
  test("an aborted run also keeps same-run context until session_stop", async () => {
    AgentSession.prototype.getContextUsage = function () {
      return { tokens: 1_000, contextWindow: 100_000, percent: 1 };
    };
    ...
    const fixture = createFixture(branch.sessionManager, {
      ...
      beforeAcmRegistration(api) {
        api.on("session_stop", () => {
          ...
          // sees the same session_stop event.
          idle = false;
        });
      },
    });

...
describe("deferred live synchronization fallback", () => {
  test("unavailable native synchronization defers to session_stop and then uses persistent rebuild", async () => {
  ...
  test("replacement failure is observed only at session_stop and persistent rebuild remains recoverable", async () => {
    const { rootId, sessionManager } = createSession();

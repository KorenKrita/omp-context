...
describe("repeated travel, restoration, and resume with deferred delivery", () => {
  ...
  test("resume keeps persisted branch state and a new travel settles against the resumed manager", async () => {
    ...
    const resumedManager = await SessionManager.open(sessionFile, directory);
    ...
    await emit(resumedFixture.handlers, "session_stop", {}, resumedFixture.context);
  ...
  test("resume uses the persisted session header cwd when no override is supplied", async () => {
    ...
    const headerIndex = lines.findIndex((line) => JSON.parse(line).type === "session");
    if (headerIndex < 0) throw new Error("Expected a session header entry");
    const header = JSON.parse(lines[headerIndex] ?? "null");
    lines[headerIndex] = JSON.stringify({ ...header, cwd: armWorkspace });
    writeFileSync(sessionFile, lines.join("\n") + "\n");

    const resumed = await SessionManager.open(sessionFile, root);

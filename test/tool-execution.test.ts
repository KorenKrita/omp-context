...
function successfulTravelContext(
  ...
  const sessionManager = {
    ...
    getBranch: (fromId?: string) => {
      ...
      const activeRoot = postMutationPacketInvalid ? invalidRoot : root;
      if (fromId === root.id) return [activeRoot];
      return leafId === head.id ? [activeRoot, head] : [activeRoot, entries.at(-1)!];
    },
    getLeafId: () => leafId,
    getEntry: (id: string) => entries.find((entry) => entry.id === id),
    appendLabelChange: (targetId: string, label: string | undefined) => {
    ...
    branchWithSummary: (targetId: string, summary: string, details: unknown, fromExtension?: boolean) => {
      branchCalls++;
      // Mirror the real host contract: fromId is the branch base (not the
      // pre-travel leaf) and fromExtension echoes the caller's flag.
...
function midSpanDamagedTravelContext(toolCallId: string) {
  ...
  // OMP host adaptation: the OMP buildSessionContext projection strips a
  // bare dangling toolCall entirely, so a prefix ending at the dangling
  // assistant would rebuild as "complete" here (on Pi it stays "repaired").
  // Ordering the stale orphan result BEFORE the dangling assistant keeps the
  // field-failure semantics identical on this host: every candidate strictly
  // after the clean boundary carries the orphan and rebuilds as "repaired".
  const orphanResult = {
    type: "message",
    id: "mid-span-orphan",
  ...
  const laterWorkA = userEntry("mid-span-later-a", danglingAssistant.id, "2026-01-01T00:00:04.000Z");
  ...
  const entries: SessionEntry[] = [root, cleanTurn, orphanResult, danglingAssistant, laterWorkA, laterWorkB, travelCall];
...
describe("ACM tool execution contracts", () => {
  ...
  test("uses a host-normalized current packet instead of rejecting raw duplicate tool calls", async () => {
    // OMP's session manager normalizes duplicate tool calls when the packet is
    // rebuilt, so a raw duplicate never surfaces as current_protocol_invalid;
    // the travel proceeds and stops at the fixture's failing label capability.
    ...
    expect(fixture.getAppendCalls()).toBe(1);
    expect(fixture.getBranchCalls()).toBe(0);
  ...
  test("treats post-mutation host-normalized evidence as verified with the return ticket intact", async () => {
    const result = await executeTravel(
    ...
    expect(result.details).toMatchObject({
      ...
      postMutationProtocolStatus: "complete",
      postMutationProtocolDefects: [],
      postMutationProtocolRepairs: [],
      // The return-ticket transaction rides on every applied receipt: trusted
      // receipt matching and [raw archive] classification read these fields
    ...
    expect(result.content[0]?.text).toContain("Travel complete");
    expect(result.content[0]?.text).not.toContain("invalid_tool_call_id");
    expect(result.content[0]?.text).toContain(`Applied handoff NEXT: ${HANDOFF.next}`);
  });

  test("an applied receipt forms a trusted travel transaction", async () => {
    ...
    expect(result.details).toMatchObject({
      mutationStatus: "applied",
      // OMP host normalization keeps the post-mutation packet valid.
      postMutationEvidenceStatus: "verified",
    });

...
export function registerTravelTool(pi: ExtensionAPI, runtime: AcmSessionRuntime): void {
  const handoffFields = {
    ...
    evidence: pi.zod.string().describe(HANDOFF_FIELD_DESCRIPTIONS.evidence).optional(),
    external: pi.zod.string().describe(HANDOFF_FIELD_DESCRIPTIONS.external).optional(),
    exclusions: pi.zod.string().describe(HANDOFF_FIELD_DESCRIPTIONS.exclusions).optional(),
    recover: pi.zod.string().describe(HANDOFF_FIELD_DESCRIPTIONS.recover).optional(),
  };
  const schema = pi.zod.object({
    target: pi.zod.string().min(1).describe(
      "Where to return to: checkpoint name, node ID, or 'root'. Pick the point immediately before the material being folded — the checkpoints view lists candidates with projected gains.",
    ),
    ...
    backupCurrentHeadAs: pi.zod.string().min(1).regex(/^[A-Za-z0-9._-]+$/).describe(
      "Optional unique custom name for the automatic return ticket; 'root' is reserved. Omit to use the name derived from the handoff goal.",
    ).optional(),
  }).strict();

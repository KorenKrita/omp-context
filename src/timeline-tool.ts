  ...
  registerTool({
    ...
    async execute(
      ...
      if (params.view === "checkpoints") {
        ...
        lines.push(`Checkpoints (${listings.length} matching aliases, ${checkpointsDisplayedAliases} displayed${filter ? ` for '${sanitizeTerminalText(params.filter)}'` : ""}; cap ${params.limit}). Current: ${currentResult.value.length} msgs, ${formatContextUsage(usage, true)}, summary depth ${activeSummaryDepth}:`);

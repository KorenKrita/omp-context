...
export function registerAcmLifecycle(pi: ExtensionAPI, runtime: AcmSessionRuntime): void {
  ...
  pi.on("tool_execution_end", (event, ctx: ExtensionContext) => {
    ...
    // and live message sequence; session_stop owns the actual replacement.
  ...
  pi.on("session_stop", (_event, ctx: ExtensionContext) => {
    ...
    try {
      if (ctx.isIdle() === false || ctx.hasPendingMessages()) return;
    } catch {
      return;
    }
  ...
  pi.on("context", (event, ctx: ExtensionContext) => {
    ...
    // AgentSession messages still wait for session_stop.
    //
  ...
  // OMP exposes overflow retry evidence on auto_compaction_end rather than
  // session_compact. Repair only the live tail; persisted history is untouched.
  pi.on("auto_compaction_end", (event, ctx: ExtensionContext) => {
